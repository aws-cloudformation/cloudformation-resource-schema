...
* **`propertyTransform`**: Is a map (Map<String, String>) with the keys being property paths and values being JQ transformation functions (https://stedolan.github.io/jq/). This property is used to avoid falsely drifted resources. If the handler transforms the input to the resource to an expected value a transform function can be defined for this property to avoid drift.
#### Application
...
```
{
    ...
    "propertyTransform": {
        "/properties/Id": ".Id | sqrt $OR .Id + 1",
        "/properties/NestedProperty/Expression": "\"Prefix\" + .NestedProperty.Expression"
    }
}
```
