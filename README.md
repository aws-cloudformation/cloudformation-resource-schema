...
* **`propertyTransform`**: Is a map (Map<String, String>) with the key set to property name and value to set to jsonata transformation function (https://jsonata.org/). This property is used to avoid falsely drifted resources. If the handler transforms the input to the resource to an expected value a transform function can be defined for this property to avoid drift.
#### Application
